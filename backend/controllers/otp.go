...
func InitOTPCollection(db *mongo.Database) {
	...
	otpCollection.Indexes().CreateOne(ctx, mongo.IndexModel{
...
func SendOTPHandler(c *gin.Context) {
	...
	// 既存の未使用OTPを無効化
	otpCollection.UpdateMany(ctx, bson.M{
		"user_id": user.ID,
		"purpose": req.Purpose,
		"is_used": false,
	}, bson.M{
...
func VerifyOTPHandler(c *gin.Context) {
	...
	services.StoreOTPUsage(user.ID.Hex(), req.Purpose, 60*time.Second)

	// 目的に応じた処理
	switch req.Purpose {
	case "login":
		accessToken, csrfToken, expiresIn, err := issueTokens(c, user)
		if err != nil {
			// トークン生成に失敗した場合、OTPの使用済みマークを取り消す
			otpCollection.UpdateOne(ctx, bson.M{"_id": otp.ID}, bson.M{
...
// generateAccessToken はユーザー用のJWTトークンを生成
