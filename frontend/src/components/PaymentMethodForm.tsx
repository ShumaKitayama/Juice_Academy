...
const translateStripeError = (errorMessage: string): string => {
  const errorMap: { [key: string]: string } = {
    ...
    'Your card\'s expiration date is incomplete.': 'カードの有効期限が不完全です。',
    'Your card\'s expiration date is invalid.': 'カードの有効期限が無効です。',
    'Your card\'s security code is incomplete.': 'セキュリティコードが不完全です。',
    ...
    'Your card\'s security code is incorrect.': 'セキュリティコードが正しくありません。',
    'Your card does not support this type of purchase.': 'お使いのカードはこのタイプの購入をサポートしていません。',
...
const PaymentMethodForm: React.FC<PaymentMethodFormProps> = ({ onSuccess }) => {
  ...
  useEffect(() => {
    const getSetupIntent = async () => {
      ...
      try {
        ...
        setError(err.response?.data?.error || 'SetupIntentの取得に失敗しました');
      }
    };
  ...
  const handleCardChange = (event: any) => {
    setCardError(event.error ? translateStripeError(event.error.message) : null);
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (stripeError) {
      setError(translateStripeError(stripeError.message || 'カード情報の登録に失敗しました'));
      setProcessing(false);
      return;
    }
    ...
    if (setupIntent && setupIntent.status === 'succeeded') {
      try {
        // バックエンドに支払い方法の登録を通知
        await paymentAPI.confirmSetup(user.id, setupIntent.payment_method as string);
        setSucceeded(true);
        onSuccess();
      } catch (err: any) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in">
      <div className="bg-white p-6 rounded-lg shadow-md card-payment-form">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-medium text-gray-900">クレジットカード情報</h3>
          <div className="flex space-x-2">
            ...
            <img src="https://cdn.jsdelivr.net/gh/creativetimofficial/public-assets@master/soft-ui-design-system/assets/img/logos/amex.png" alt="amex" className="h-6" />
          </div>
        </div>
        
        {error && <ErrorAlert message={error} className="animate-slide-up" />}
        
        {succeeded && <SuccessAlert message="カード情報が正常に登録されました" className="animate-slide-up" />}
        
        <div className="mb-6">
          <label htmlFor="card-element" className="block text-sm font-medium text-gray-700 mb-2">
            カード情報
          </label>
          <div className={`border rounded-md p-4 bg-gray-50 transition-all ${
            cardError 
          ...
          {cardError && (
            <p className="mt-2 text-sm text-red-600 animate-slide-up">{cardError}</p>
          )}
          <div className="mt-3 text-xs text-gray-500 space-y-1">
            <p>* セキュリティのため、カード情報は当サイトのサーバーには保存されません。</p>
            <p>* テスト用カード番号: 4242 4242 4242 4242 (有効期限: 任意の未来日付、CVC: 任意の3桁)</p>
          </div>
        ...
        <Button
          ...
          disabled={processing || !stripe || !clientSecret || succeeded || !!cardError}
          fullWidth
